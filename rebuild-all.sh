...
if [ ! -f packages/linux-image-*-vyos_*.deb ]; then 
   ...
   (cd packages/vyos-kernel;LOCALVERSION= make-kpkg --rootcmd fakeroot --initrd --append_to_version -amd64-vyos --revision=mickvav kernel_source kernel_debug kernel_headers kernel_manual kernel_doc kernel_image)
fi;
